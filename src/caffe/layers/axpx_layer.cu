...
__global__ void AxpxForward(const int count, const int spatial_dim, 
    const Dtype* scale_data, const Dtype* x_data, Dtype* out_data) {
...
template <typename Dtype>
void AxpxLayer<Dtype>::Forward_gpu(
  ...
  AxpxForward<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
      count, bottom[1]->count(3), scale_data, x_data, out_data);  
}

template <typename Dtype>
__global__ void AxpxBackwardScale(const int outer_num, const int spatial_dim, 
...
template <typename Dtype>
__global__ void AxpxBackwardX(const int count, const int spatial_dim, 
...
template <typename Dtype>
void AxpxLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  const int count = top[0]->count();
  const Dtype* top_diff = top[0]->gpu_diff();
  if (propagate_down[0]) {
    int outer_num = bottom[1]->count(0, 3);
    AxpxBackwardScale<Dtype><<<outer_num, CAFFE_CUDA_NUM_THREADS>>>(
        outer_num, bottom[1]->count(3),
        bottom[1]->gpu_data(), top_diff,
        bottom[0]->mutable_gpu_diff()); 
  }
  if (propagate_down[1]) {
    AxpxBackwardX<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        count, top[0]->count(3), 
...
INSTANTIATE_LAYER_GPU_FUNCS(AxpxLayer);
