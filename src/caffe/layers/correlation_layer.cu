...
template <typename Dtype> 
__global__ void CorrelateDataMultiply(const int nthreads, int num, int item, int topwidth, int topheight, int topchannels, int topcount,
      ...
      const vector<Blob<Dtype>*>& top)
{
    ...
    if(corr_type_ == CorrelationParameter_CorrelationType_MULTIPLY) {
        ...
        for (int n = 0; n < num; n++) {
          ...
          CorrelateDataMultiply<Dtype><<<CAFFE_GET_BLOCKS(topThreadCount), CAFFE_CUDA_NUM_THREADS>>>(
              topThreadCount,
