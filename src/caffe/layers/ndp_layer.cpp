...
#include "caffe/layers/ndp_layer.hpp"
...
void NDPLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  NDPParameter pool_param = this->layer_param_.ndp_param();
  ...
  if (pad_sum != 0 ) {
     CHECK(this->layer_param_.ndp_param().pool() == NDPParameter_PoolMethod_AVE
        || this->layer_param_.ndp_param().pool() == NDPParameter_PoolMethod_MAX)
...
template <typename Dtype>
void NDPLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  NDPParameter pool_param = this->layer_param_.ndp_param();
  channel_axis_ = bottom[0]->CanonicalAxisIndex(pool_param.axis());
  ...
  if (this->layer_param_.ndp_param().pool() ==
      NDPParameter_PoolMethod_MAX && top.size() == 1) {
    max_idx_.Reshape(top_shape);
  }
  // If stochastic pooling, we will initialize the random index part.
  if (this->layer_param_.ndp_param().pool() ==
      NDPParameter_PoolMethod_STOCHASTIC) {
...
// case?
template <typename Dtype>
void NDPLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  switch (this->layer_param_.ndp_param().pool()) {
  case NDPParameter_PoolMethod_MAX:
    // Initialize
    if (use_top_mask) {
    ...
    break;
  case NDPParameter_PoolMethod_AVE:
    ...
    break;
  case NDPParameter_PoolMethod_STOCHASTIC:
    NOT_IMPLEMENTED;
    break;
  default:
...
template <typename Dtype>
void NDPLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
INSTANTIATE_CLASS(NDPLayer);
REGISTER_LAYER_CLASS(NDP);
}  // namespace caffe